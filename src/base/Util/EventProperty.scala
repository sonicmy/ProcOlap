...
case class EventProperty (name: String, datatype: String, values: List[String], isExt: Boolean) {
    ...
    i =>
      ...
      label.setOnMouseReleased(new EventHandler[MouseEvent] {
        def handle(p1: MouseEvent) {
          if(isExt) setSQL("AND \"EVENT\".\""+name.toUpperCase+"\" #oper# '"+i+"' ")
          else setSQL("AND \"EPROPERTY\".\"KEY\" = '"+name+"' AND \"EPROPERTY\".\"VALUE\" #oper# '"+i+"' ")
        }
      })
  ...
  def setBox(maxPages: Int, labels: List[Label] ) {
    if(chooser != null) chooser.setLabels(labels,label,datatype)
  }
